...
export function loadSimulation(app) {
    ...
    document.getElementById("simulate-create").addEventListener("click", async () => {
        try {
            ...
            if (!response.ok) throw new Error("Failed to simulate delivery creation");
    ...
    function addDeliveryRow(delivery) {
        ...
        row.innerHTML = `
            ...
            <td>${delivery.faktiskLevering ? "Completed" : "Pending"}</td>
        `;

        const completeButton = row.querySelector("button");
        completeButton.addEventListener("click", () => completeDelivery(delivery.leveringId, row));
    ...
    async function completeDelivery(leveringId, row) {
        try {
            ...
            row.children[7].textContent = `Completed at ${new Date(completedDelivery.faktiskLevering).toLocaleString()}`;
            row.querySelector("button").remove(); // Remove the complete button
        } catch (error) {
            console.error(error);
